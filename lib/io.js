...
module.exports = app => {
  ...
  app.beforeStart(async () => {
    for (const nsp in namespace) {
      ...
      if (connectionMiddlewareConfig) {
        ...
        for (const name of connectionMiddlewareConfig) {
          // 不允许所注册的中间件在 app/middleware 和 app/io/middleware 目录重复，否则不知道注册哪个
          assert(!(app.io.middleware[name] && app.middleware[name]), `duplicate middleware: ${name} in app/middleware and app/io/middleware dir!`);

          const middleware = app.io.middleware[name] || app.middleware[name];
          assert(middleware, `can't find middleware: ${name} in app/middleware and app/io/middleware dir!`);
          connectionMiddlewares.push(middleware(app));
        }
      }

      if (packetMiddlewareConfig) {
        assert(is.array(packetMiddlewareConfig), 'config.packetMiddleware must be Array!');
        for (const name of packetMiddlewareConfig) {
          // 不允许所注册的中间件在 app/middleware 和 app/io/middleware 目录重复，否则不知道注册哪个
          assert(!(app.io.middleware[name] && app.middleware[name]), `duplicate middleware: ${name} in app/middleware and app/io/middleware dir!`);

          const middleware = app.io.middleware[name] || app.middleware[name];
          assert(middleware, `can't find middleware: ${name} in app/middleware and app/io/middleware dir!`);
          packetMiddlewares.push(middleware(app));
        }
      }
  ...
  app.on('server', server => {
    app.io.attach(server, config.options);
